...
# dimensional change : 2 X 2 - output
...
with(data1,plot(x=dd,y=Global_active_power,col="black", type="l", xlab="",
                ylab="Global Acitve Power (kilowatts)", lwd=0.5, xaxt="n"
))
axis.POSIXct(side=1,at=dates,format="%a")

# plot 2

with(data1,plot(x=dd,y=Voltage,col="black", type="l", xlab="datetime",
                ylab="Voltage", lwd=0.5, xaxt="n"
))
axis.POSIXct(side=1,at=dates,format="%a")
...
with(data1,plot(x=dd,y=Global_reactive_power,col="black", 
                type="S", xlab="datetime",
                ylab="Global_reactive_power", lwd=0.3, xaxt="n"
))
axis.POSIXct(side=1,at=dates,format="%a")
